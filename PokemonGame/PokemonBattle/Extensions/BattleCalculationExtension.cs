...
public static class BattleCalculationExtension
{
    ...
    public static Answer CalculateDamage(this Battle battle, Pokemon attackingPokemon, Pokemon defendingPokemon, bool playerTurn, Move move)
    {
        ...
        if (move.IsStatus) return new Answer(AnswerCodes.AnswerCalculationStatusMove, 0); // Status Move

        bool isCriticalHit = CriticalHitCheck(attackingPokemon, defendingPokemon, move);
        answerCode = isCriticalHit ? AnswerCodes.AnswerCalculationCriticalHit : AnswerCodes.AnswerOk;
    ...
    private static double CalculateMovePower(Battle battle, IPokemonParty attackingParty, BattlerSide defenderSide, Pokemon attacker, Pokemon defender, Move move) 
    {
